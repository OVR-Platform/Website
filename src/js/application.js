...
var ten_days=1000*60*60*24*10;var end=new Date("12/1/2018");var timer=_countdownJs2.default.timer(end,function(timeLeft){document.getElementById("c-countdown_container").innerHTML="Presale will start in "+timeLeft.days+" days "+timeLeft.hours+" hours "+timeLeft.minutes+" min "+timeLeft.seconds+" sec ";},function(){console.log("Countdown Finished!");});};var onMobileHamburgerClick=function onMobileHamburgerClick(){var pageWrap=(0,_util.$1)("#hamburger-menu");var menuVoices=(0,_util.$)(".c-mobile-nav__container-main a");pageWrap.addEventListener("click",function(){if(body.classList.contains("is_mobile_menu_open")){body.classList.remove("is_mobile_menu_open");}else{body.classList.add("is_mobile_menu_open");}});(0,_util.each)(menuVoices,function(i,menuVoice){menuVoice.addEventListener("click",function(){body.classList.remove("is_mobile_menu_open");});});};var preloaderFadeOut=function preloaderFadeOut(){body.classList.add("c-loader--logo-holder-animated");setTimeout(function(){body.classList.remove("c-loader--logo-holder-animated");setTimeout(function(){body.classList.add("c-loader--preloader-white-gone");},1600);},1850);};var scrollToLink=function scrollToLink(){var scroll=new _smoothScroll2.default('a[href*="#"]',{speed:500,offset:100});};var init=function init(){scrollToLink();startParallax();initCarousels();menuManager();countDownManager();(0,_util.listenForms)();onMobileHamburgerClick();preloaderFadeOut();};return{init:init};}();exports.default=Application;
...
(0,_reqwest2.default)({url:'/mailer/mailer.php',method:'post',data:{name:form.querySelector('.name').value,email:form.querySelector('.email').value,msg:form.querySelector('.msg').value},success:function success(resp){console.log(resp);setTimeout(function(){if(resp=="false"){$1("body").classList.remove("is_form--loading");$1("body").classList.add("is_form--error");}else{$1("body").classList.remove("is_form--loading");$1("body").classList.add("is_form--success");}setTimeout(function(){$1("body").classList.remove("is_form--success");$1("body").classList.remove("is_form--error");$1("body").classList.remove("is_form--loading");form.querySelector('.name').value="";form.querySelector('.email').value="";form.querySelector('.msg').value="";couldSendEmail=true;},3000);},500);}});},4000,true);function listenForms(){var forms=$('.c-general-contact-form');// console.log('listenForms','listenForms')
// Forms
each(forms,function(i,form){if(!form.classList.contains("is_form--listening")){// console.log('form_di_invio_appeso_per', form)
form.addEventListener("submit",function(e){e.preventDefault();console.log('ferma');submitForm(form);},true);form.classList.add("is_form--listening");}});each($('.c-contact-form'),function(i,cf){cf.querySelector(".privacy").addEventListener("change",function(){if(this.checked){cf.querySelector(".submit-button").classList.remove("disabled-2");}else{cf.querySelector(".submit-button").classList.add("disabled-2");}});// cf.querySelector(".disabled-2").addEventListener("click", function(){
